...
trait ControllerAdviceProcessor {
  ...
  def handleError(request: mvc.RequestHeader, ex: Throwable): Option[Future[Result]] = {
    ...
    def defineHandler(e: Exception): Option[Future[Result]] = {
      ...
      resolvers find {case (advice, resolver) =>
        ...
        case Some((advice, resolver)) =>
          ...
          //if returning type is not Result
          if (!classOf[Result].isAssignableFrom(method.getDeclaringClass) &&
            //if @ResponseBody is not present
            !method.isAnnotationPresent(classOf[ResponseBody])){
            //then fire a fail
            throw new NotImplementedError("Right now only @ResponseBody handlers are supported for any returning type, except Result.")
          }
